...
public final class ModalUtils {
    ...
    public static boolean isAliasUnique(final String alias, List<ListItemDTO> attributes) {
        for (final ListItemDTO listItemDTO : attributes) {
            if ((listItemDTO.getAlias().equalsIgnoreCase(alias) || listItemDTO.getAttributeDescriptor().getQualifier().equalsIgnoreCase(alias)) && listItemDTO.getAlias().equals(listItemDTO.getAttributeDescriptor().getQualifier())) {
                return false;
            }
        }
        return true;
