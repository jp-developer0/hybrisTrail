...
public class KymaUpdateConsumedDestinationsJobPerformable extends AbstractJobPerformable<CronJobModel>
{
	...
	public PerformResult perform(final CronJobModel cronJob)
	{
				...
				&& destinationTarget.getRegistrationStatus() == RegistrationStatus.IN_PROGRESS)
		{
			final String errorMessage = String.format(
					"Could not retrieve required urls within [{%s}] minutes, integration failed. Destination target with id: [{%s}] registration status is set to Error.",
					destinationTarget.getId(), Config.getInt(CRONJOB_LIFESPAN_PROP_KEY, DEFAULT_CRONJOB_LIFESPAN));
	...
	{
		return Stream.of(urls.getEventsUrl(), urls.getMetadataUrl(), urls.getRenewCertUrl()).anyMatch(StringUtils::isEmpty);
	}
