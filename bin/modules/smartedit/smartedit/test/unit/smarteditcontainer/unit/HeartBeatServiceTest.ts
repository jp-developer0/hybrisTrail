...
describe('Storefront <-> SmartEdit Heart beat service', () => {
	...
	function createHeartBeatServiceInstance(heartbeatTimeout: number = 10000) {
		...
		mockAlertFactory = jasmine.createSpyObj('alertFactory', ['createWarning', 'createInfo']);
		mockAlertFactory.createWarning.and.callFake(() => mockDisconnectedAlert);
		mockAlertFactory.createInfo.and.callFake(() => mockReconnectedAlert);
	...
	it('On $routeChangeStart will reset and hide all alerts', () => {
		...
		// When
		mockRootScopeService.triggerEvent('$routeChangeStart');
	...
	it('On timerElapsed reconnecting is shown, then heartBeat event will show reconnected', () => {
		...
		// When
		mockCrossFrameEventService.triggerEvent(MOCK_EVENTS.PAGE_CHANGE);   // trigger very short timer
		...
		mockMessageGateway.triggerEvent(HeartBeatService.HEART_BEAT_MSG_ID);
	...
	it('On $routeChangeSuccess will not start heartbeat timer for non-storefront path', () => {
		...
		// When
		mockRootScopeService.triggerEvent('$routeChangeSuccess');
	...
	it('On PAGE_CHANGE will trigger a restart of the heartbeat timer', () => {
		...
		// When
		mockCrossFrameEventService.triggerEvent(MOCK_EVENTS.PAGE_CHANGE);
