...
public class SelectiveCartLoaderStrategy extends DefaultCartLoaderStrategy
{
	...
	{
		final String selectiveCartId = SELECTIVE_CART + ((CustomerModel) getUserService().getCurrentUser()).getCustomerID();

		if (selectiveCartId.equals(cartID))
		{
			final Optional<CartModel> optionalCart = getSelectiveCartService()
					.getSelectiveCartForUser(getUserService().getCurrentUser());
			if (!optionalCart.isPresent() || !isBaseSiteValid(optionalCart.get()))
			{
			...
			checkCartExpiration(cartID, getCartService().getSessionCart().getCode());
		}
		else
		{
