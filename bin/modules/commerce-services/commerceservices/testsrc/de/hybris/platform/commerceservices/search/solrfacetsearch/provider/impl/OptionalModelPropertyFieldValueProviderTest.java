...
public class OptionalModelPropertyFieldValueProviderTest extends PropertyFieldValueProviderTestBase
{
	...
	public void testWhenIndexPropertyIsLocalized() throws FieldValueProviderException
	{
		...
		given(fieldNameProvider.getFieldNames(Matchers.<IndexedProperty> any(), eq(TEST_EN_LANG_CODE)))
				.willReturn(singletonList(TEST_PROP_FIELD_NAME_EN));
		given(fieldNameProvider.getFieldNames(Matchers.<IndexedProperty> any(), eq(TEST_DE_LANG_CODE)))
				.willReturn(singletonList(TEST_RATING_DOUBLE_VAL));
		...
		for (final FieldValue val : result)
		{
			...
			Assert.assertTrue(
					val.getFieldName().equalsIgnoreCase(TEST_PROP_FIELD_NAME_EN) || val.getFieldName().equals(TEST_RATING_DOUBLE_VAL));
		}
	...
	public void testAttributeNotSupportedException() throws FieldValueProviderException
	{
		...
		given(modelService.getAttributeValue(baseProduct, getPropertyName()))
				.willThrow(new AttributeNotSupportedException("cannot find attribute size", TEST_PROP));
