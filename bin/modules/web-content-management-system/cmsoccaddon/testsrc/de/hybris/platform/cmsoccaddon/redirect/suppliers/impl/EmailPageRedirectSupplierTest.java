...
public class EmailPageRedirectSupplierTest
{
	...
	public void setup()
	{
		when(typeCodePredicate.test(EmailPageModel._TYPECODE)).thenReturn(true);
		when(typeCodePredicate.negate()).thenReturn(typeCodeNegatedPredicate);
		when(typeCodeNegatedPredicate.test(EmailPageModel._TYPECODE)).thenReturn(false);
	...
	public void shouldPopulateAllParametersForRedirectUrl()
	{
		...
		assertThat(pageTypes, Matchers.containsInAnyOrder(EmailPageModel._TYPECODE));
