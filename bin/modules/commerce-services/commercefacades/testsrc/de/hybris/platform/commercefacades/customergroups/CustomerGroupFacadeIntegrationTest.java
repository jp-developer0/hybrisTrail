...
public class CustomerGroupFacadeIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void testTryCreateCustomerGroupWhenSuperGroupDoesNotExist()
	{
		...
		try
		{
			TestUtils.disableFileAnalyzer(
					"Creating customer group when group with id 'customergroup' does not exists should throw exception");
			customerGroupFacade.createCustomerGroup(UID, LOCALIZED_NAME);
		}
		finally
		{
	...
	@Test
	public void testGetCustomerGroupForCurrentUser()
	...
	public void testTryToGetSubGroupsOfGroupWhichDoesNotExist()
	{
		...
		{
			TestUtils.disableFileAnalyzer(
					"customerGroupFacade.getSubGroupsOfCustomerGroup should throw exception when pass uid of group which does not exists");
			customerGroupFacade.getCustomerGroup("groupName" + System.currentTimeMillis(),
					Collections.singleton(UserGroupOption.BASIC));
			Assert.fail(
					"customerGroupFacade.getSubGroupsOfCustomerGroup should throw exception when pass uid group which does not exists");
		}
	...
	public void testTryToGetSubGroupOfGroupWhichIsNotCustomerGroup()
	{
		...
		{
			TestUtils.disableFileAnalyzer(
					"customerGroupFacade.getSubGroupsOfCustomerGroup should throw exception when pass uid of group which is not customer Group");
			customerGroupFacade.getCustomerGroup(userGroup.getUid(), Collections.singleton(UserGroupOption.BASIC));
			Assert.fail(
					"customerGroupFacade.getSubGroupsOfCustomerGroup should throw exception when pass uid of group which is not customer Group");
		}
		catch (final InvalidCustomerGroupException ex)
		{
			//throwing exception is expected behavior
		}
		finally
