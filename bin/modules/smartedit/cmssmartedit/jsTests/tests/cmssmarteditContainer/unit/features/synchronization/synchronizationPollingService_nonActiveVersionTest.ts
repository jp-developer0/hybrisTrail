...
describe("Synchronization polling service with content catalog non active version - ", () => {
	...
	it('when syncStatus in the scope is empty then getSyncStatus will fetch the sync status by making a rest call and set it to the scope object ', () => {
		...
		// WHEN
		const promise = syncPollingService.getSyncStatus(MOCK_PAGE_ID_1);
	...
	it('when getSyncStatus is called multiple times, timer does not restart more than once', function() {
		// GIVEN
		pageInfoService.getPageUUID.and.returnValue($q.when(MOCK_PAGE_ID_1));
		pageSynchronizationGetRestService.get.and.returnValue($q.when(pageId1SyncStatus));
		...
		// WHEN
		syncPollingService.getSyncStatus(MOCK_PAGE_ID_1);
		syncPollingService.getSyncStatus(MOCK_PAGE_ID_1);
		syncPollingService.getSyncStatus(MOCK_PAGE_ID_1);
	...
	describe('fetchSyncStatus', () => {
		...
		it('GIVEN new sync status WHEN fetchSyncStatus is called THEN it will publish a FAST_FETCH event', () => {
			...
			// THEN
			expect(crossFrameEventService.publish).toHaveBeenCalledWith(SYNCHRONIZATION_POLLING.FAST_FETCH, pageId1SyncStatus);
		});
		...
		it('GIVEN first poll WHEN fetchSyncStatus is called THEN it will publish a PAGE_UPDATED event', () => {
			...
			// THEN
			expect(crossFrameEventService.publish).toHaveBeenCalledWith(SYNCHRONIZATION_POLLING.FAST_FETCH, pageId1SyncStatus);
			expect(crossFrameEventService.publish).toHaveBeenCalledWith(EVENTS.PAGE_UPDATED, {uuid: MOCK_PAGE_ID_1});
		});
		...
		it('GIVEN page information did not change from last time it was synched WHEN fetchSyncStatus is called THEN no page updated event is triggered', () => {
			...
			// THEN
			expect(crossFrameEventService.publish).toHaveBeenCalledWith(SYNCHRONIZATION_POLLING.FAST_FETCH, pageId1SyncStatus);
			expect(crossFrameEventService.publish).not.toHaveBeenCalledWith(EVENTS.PAGE_UPDATED, {uuid: MOCK_PAGE_ID_1});
		});
